    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            return Response({
                ...
                'message': 'Hey, thanks for signing up! A passcode has been sent to your email.'
...
class PasswordResetRequestView(GenericAPIView):
    ...
    def post(self, request):
        serializer = self.serializer_class(data=request.data, context={'request':request})
        serializer.is_valid(raise_exception=True)
        return Response({'message': 'A password reset link has been sent to your email. Please check your inbox to proceed.'}, status=status.HTTP_200_OK)
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                return Response({'message':'Token is invaild or has expired'}, status=status.HTTP_401_UNAUTHORIZED)
